...
=======
    '/welcome.html',
>>>>>>> 03b62b0afb23e9ba273d79b675f093001b5bf7ec
>>>>>>> 2a481b0045c6bc05e6f86a7752ced83c0348c230
    '/customer-login.html',
...
<<<<<<< HEAD
  // If on customer dashboard without customer session, redirect to landing
=======
<<<<<<< HEAD
  // If on customer dashboard without customer session, redirect to landing
  if (currentPage.includes('index.html') && !sessionId) {
    window.location.href = 'landing.html';
    return false;
  }
  
  // If on owner dashboard without owner session, redirect to landing
  if (currentPage.includes('owner.html') && !ownerSessionId) {
    window.location.href = 'landing.html';
    return false;
  }
  
  // If customer logged in and trying to access customer login/register, redirect to dashboard
=======
  // If on index.html (dashboard) without session, redirect to welcome
>>>>>>> 2a481b0045c6bc05e6f86a7752ced83c0348c230
  if (currentPage.includes('index.html') && !sessionId) {
    window.location.href = 'landing.html';
    return false;
...
<<<<<<< HEAD
>>>>>>> 2a481b0045c6bc05e6f86a7752ced83c0348c230
  // If owner logged in and trying to access owner login/register, redirect to dashboard
...
async function loadCustomerInfo() {
  ...
  try {
    const res = await fetch('/customer-info', {
...
async function checkQueuePosition() {
  ...
  try {
    const res = await fetch('/my-position', {
      headers: { 'session-id': sessionId }
    });
    ...
    if (data.inQueue) {
      ...
      if (joinQueueSection) {
        joinQueueSection.style.display = 'none';
      }
    } else {
      ...
      if (joinQueueSection) {
        joinQueueSection.style.display = 'block';
      }
    }
  } catch (error) {
...
if (leaveQueueBtn) {
  leaveQueueBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/leave-queue', {
        method: 'POST',
        headers: { 'session-id': sessionId }
      });
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch('/join', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ service, sessionId, salonId: salon })
      });

      const data = await res.json();
      alert(`Successfully joined queue! You are number ${data.position}.`);
      checkQueuePosition();
    } catch (error) {
...
// Owner code (keeping existing functionality)
...
if (ownerRegisterForm) {
  ownerRegisterForm.addEventListener('submit', async (e) => {
    ...
    submitBtn.disabled = true;
    submitBtn.textContent = 'Registering Salon...';
    
    const formData = new FormData(ownerRegisterForm);
    const data = Object.fromEntries(formData);
    
    try {
      const res = await fetch('/register-salon', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      });
      ...
      setTimeout(() => {
        submitBtn.disabled = false;
        submitBtn.textContent = 'Register Salon';
      }, 2000);
    }
  });
}
...
if (ownerLoginForm) {
  ownerLoginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const result = await res.json();
      if (res.ok) {
        localStorage.setItem('ownerSessionId', result.ownerSessionId);
        localStorage.setItem('salonName', result.salonName);
...
async function loadOwnerQueue() {
  ...
  try {
    const res = await fetch('/owner/queue', {
      headers: { 'owner-session-id': ownerSessionId }
    });
    ...
    if (salonInfo) {
      salonInfo.innerHTML = `
        <h3>${data.salonInfo.salonName}</h3>
        <p>Owner: ${data.salonInfo.ownerName}</p>
    ...
    if (data.queue.length === 0) {
      queueList.innerHTML = '<li>No customers in queue.</li>';
    } else {
      data.queue.forEach((customer, index) => {
        const li = document.createElement('li');
        li.innerHTML = `
          <strong>#${index + 1}</strong> - ${customer.name} 
          <br><small>Service: ${customer.service} | Wait: ${customer.waitTime}</small>
        `;
...
// Serve button for owners
if (nextBtn) {
  let serveInProgress = false;
  
  nextBtn.addEventListener('click', async () => {
    if (serveInProgress) return;
    
    serveInProgress = true;
    nextBtn.disabled = true;
    nextBtn.textContent = 'Serving...';
    
    const ownerSessionId = localStorage.getItem('ownerSessionId');
    
    try {
      const res = await fetch('/owner/serve', { 
        method: 'POST',
        headers: { 'owner-session-id': ownerSessionId }
      });
      ...
      setTimeout(() => {
        ...
        nextBtn.textContent = 'Serve Next Customer';
      }, 1000);
    }
  });
}
...
function addLogoutButton() {
  const ownerSessionId = localStorage.getItem('ownerSessionId');
  if (ownerSessionId && window.location.pathname.includes('owner.html')) {
...
if (window.location.pathname.includes('owner.html')) {
  // Owner dashboard
  loadOwnerQueue();
  setInterval(loadOwnerQueue, 3000);
  addLogoutButton();
}
